...
def _build_feature_frame(raw5m: pd.DataFrame, cand: dict, session_mask: dict[str, set[int]]) -> pd.DataFrame:
    ...
    out["zscore"] = (out["close"] - out["basis"]) / ((out["upper"] - out["basis"]).replace(0.0, np.nan) / boll_std)
    ...
    long_reclaim = (out["close_prev"] <= out["lower_prev"]) & (out["close"] > out["lower"])
    short_reclaim = (out["close_prev"] >= out["upper_prev"]) & (out["close"] < out["upper"])

    out["long_signal"] = (
        allow_long
        & (out["bandwidth"] >= float(cand["bandwidth_min"]))
        & long_reclaim
        & (out["z_prev"] <= -float(cand["entry_z"]))
        & (out["zscore"] <= float(cand["reclaim_z_max"]))
        & (out["rsi_prev"] <= float(cand["rsi_long_max"]))
        & (out["rsi"] >= out["rsi_prev"] + float(cand["rsi_rebound_min"]))
    )
    out["short_signal"] = (
        allow_short
        & (out["bandwidth"] >= float(cand["bandwidth_min"]))
        ...
        & (out["z_prev"] >= float(cand["entry_z"]))
        & (out["zscore"] >= -float(cand["reclaim_z_max"]))
        & (out["rsi_prev"] >= float(cand["rsi_short_min"]))
        & (out["rsi"] <= out["rsi_prev"] - float(cand["rsi_rebound_min"]))
    )

    long_exit = (out["close"] >= out["basis"]).astype(int)
    short_exit = (out["close"] <= out["basis"]).astype(int)
    tp_confirm_bars = int(cand["tp_confirm_bars"])
    out["long_exit_flag"] = long_exit.rolling(tp_confirm_bars, min_periods=tp_confirm_bars).sum().eq(tp_confirm_bars)
    out["short_exit_flag"] = short_exit.rolling(tp_confirm_bars, min_periods=tp_confirm_bars).sum().eq(tp_confirm_bars)
...
class BasisFadeHcStrategy:
    ...
    def _calc_initial_stop(self, side: str) -> float:
        if side == "LONG":
            raw = float(self.entry_band) - float(self.cfg.stop_atr_mult) * float(self.entry_atr)
            return float(self.gp3.floor_to_step(raw, float(self.rules.tick_size)))
        raw = float(self.entry_band) + float(self.cfg.stop_atr_mult) * float(self.entry_atr)
        return float(self.gp3.ceil_to_step(raw, float(self.rules.tick_size)))
    ...
    def _sync_fills(self, engine: BacktestEngine) -> None:
        ...
        for f in fills[self._seen_fills :]:
            ...
            if self.pending_entry and oid == str(self.pending_entry.get("order_id")):
                ...
                self.entry_band = float(self.pending_entry.get("signal_band", 0.0))
                side = str(self.pending_entry.get("side", ""))
                self.stop_price = self._calc_initial_stop(side)
                self.pending_entry = None
                self.pending_exit = None
                self.bars_held = 0
                self.be_triggered = False
        ...
        if qty == 0.0 and self.pending_entry is None and self.pending_exit is None and self.cur_idx >= self.cooldown_until:
            ...
            if long_sig != short_sig:
                ...
                if side == "LONG":
                    ...
                    signal_band = float(row["lower"])
                else:
                    px = self.gp3.ceil_to_step(
                        float(bar.open) * (1.0 + float(self.cfg.entry_pullback_bps) / 10_000.0),
                        float(self.rules.tick_size),
                    )
                    signal_band = float(row["upper"])
                if float(px) > 0:
                    ...
                    if float(qty_v) > 0:
                        ...
                        oid = engine.place_limit(
                            ...
                            tif=TimeInForce.GTX,
                            reduce_only=False,
                            reason=f"basis_fade_entry_{side.lower()}",
                        )
                        self.pending_entry = {
                            "order_id": oid,
                            "side": side,
        ...
        if qty != 0.0 and self.pending_exit is None:
            ...
            if exit_signal or time_exit:
                ...
                oid = engine.place_limit(
                    ...
                    tif=TimeInForce.GTC,
                    reduce_only=True,
                    reason="basis_fade_exit_time" if time_exit else "basis_fade_exit_signal",
                )
                self.pending_exit = {
                    "order_id": oid,
...
def _build_grid(args) -> list[dict]:
    ...
    grids = {
        ...
        "stop_atr_mult": gp3scan._split_floats(args.stop_atr_mults),
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Search 5m basis-fade / band reversion candidates on HC")
    ...
    ap.add_argument("--out-dir", default="d:/project/hc/output/midfreq_basis_fade_search")
    ...
    ap.add_argument("--bandwidth-mins", default="0.001,0.002")
    ap.add_argument("--entry-zs", default="1.5,2.0")
    ap.add_argument("--reclaim-z-maxs", default="0.8")
    ap.add_argument("--rsi-periods", default="7")
    ap.add_argument("--rsi-long-maxs", default="35,40")
    ap.add_argument("--rsi-short-mins", default="60,65")
    ap.add_argument("--rsi-rebound-mins", default="1.0,2.0")
    ap.add_argument("--tp-confirm-bars", default="1")
    ap.add_argument("--entry-pullbacks", default="0,1")
    ap.add_argument("--entry-timeout-bars", default="1")
    ap.add_argument("--atr-periods", default="14")
    ap.add_argument("--stop-atr-mults", default="0.8,1.0")
    ap.add_argument("--breakeven-trigger-atrs", default="0.4,0.6")
    ap.add_argument("--breakeven-offset-pcts", default="0.0003")
    ap.add_argument("--position-mults", default="24")
    ap.add_argument("--cooldown-bars", default="0")
    ...
    progress_path = out_dir / f"midfreq_basis_fade_progress_{stamp}.csv"
    result_csv = out_dir / f"midfreq_basis_fade_results_{stamp}.csv"
    summary_path = out_dir / f"midfreq_basis_fade_summary_{stamp}.json"

    results: list[dict] = []
    last_feat_rows = 0
    for idx, cand in enumerate(candidates, start=1):
        feat = _build_feature_frame(raw5m, cand, _resolve_session_mask(str(cand["session_mask"])))
        ...
        strategy = BasisFadeHcStrategy(feat, cfg, rules, gp3, bar_ms=interval_minutes * 60_000)
        ...
        print(
            ...
            f"bw={row['bandwidth_min']:.3f} rsi={row['rsi_long_max']:.0f}/{row['rsi_short_min']:.0f} "
            f"rebound={row['rsi_rebound_min']:.1f} stop_atr={row['stop_atr_mult']:.2f} hold={row['max_hold_bars']}"
        )
